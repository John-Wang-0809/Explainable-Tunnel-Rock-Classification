...
class yolo_heatmap:
    ...
    def draw_detections(self, box, color, name, img):
        ...
        cv2.rectangle(img, (xmin, ymin), (xmax, ymax), tuple(int(x) for x in color), 2)  # Draw detection box
        cv2.putText(img, str(name), (xmin, ymin - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.8, tuple(int(x) for x in color), 2,
                    lineType=cv2.LINE_AA)  # Draw class and confidence
        return img
...
def get_params():
    params = {
        'weight': 'yolo11n.pt',  # Now only need to specify weights, no need to specify cfg
        'device': 'cuda:0',
        ...
        'ratio': 0.02,  # 0.02-0.1
        'show_result': True,  # Set to False if you don't need to draw results
        'renormalize': False,  # Set to True if you need to limit heatmap within boxes (only effective for detect, segment, pose)
